...
def visualize_and_save_graph(
    ...
    plt.title(title, fontsize=20, pad=24)
    plt.axis("off")
    ...
    plt.legend(handles=legend_elements, loc="upper right", bbox_to_anchor=(1.2, 1.0), fontsize=12)
    ...
    if edge_colors:
        ...
        sm.set_array([])
        cbar = plt.colorbar(sm, shrink=0.65, pad=0.02)
...
def export_interactive_graph(
    ...
    net = Network(height="900px", width="100%", bgcolor="#ffffff", font_color="#2c3e50")
    net.heading = title
    net.barnes_hut()
